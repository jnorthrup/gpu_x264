...
x264_t *x264_encoder_open   ( x264_param_t *param )
{
    ...
    if( h->param.psz_dump_yuv )
    {
        ...
        {
            x264_log( h, X264_LOG_ERROR, "can't write to ggggf.yuv\n" );
            x264_free( h );
            return NULL;
        }
    }
...
static void x264_slice_write( x264_t *h )
{
    ...
    // ERICH: time needed to calc one slice (X slices corresponds to X threads, single threaded one slice)
   // x264_timer_start();
#ifdef GPU_OPTIMIZE
    ...
    //printf("processing frame index: %d\n",frameIndex);

	if(frameIndex > 0) {
		//x264_block(h,gpuThread,0);
	//if(gpuThread->result == 0) printf("UPDATE!!!!!!!!!!!\n");
		...
		if(h->sh.i_type == SLICE_TYPE_I) {
			printf("I FRAME FOUND so I will reset the dctRdPosition to %d ",gpuThread->dctRdIdx);
			gpuThread->dctRdIdx = (gpuThread->dctRdIdx+1) & THREAD_BUFFER_DEPTH_MASK;
			printf(" to %d\n",gpuThread->dctRdIdx);
		}
	} //else {
		++frameIndex;
	...
	if(i_mb_x == 0) {
		int test_index=0;
		test_index += ((gpuThread->dct_delta+3) <= i_mb_y && i_mb_y < (gpuThread->dct_delta+3)*2)*1;
		test_index += ((gpuThread->dct_delta+3)*2 <= i_mb_y && i_mb_y < (gpuThread->dct_delta+3)*3)*2;
		test_index += ((gpuThread->dct_delta+3)*3 <= i_mb_y && i_mb_y < (gpuThread->dct_delta+3)*4)*3;

		if(index < test_index) {
			x264_gpu_prepush_sub16x16_dct(h,gpuThread,index);
			index++;
		}
	}
#endif
         ...
         * */
        // ERICH: does transformation here too if needed
   ...
   /*#ifdef GPU_OPTIMIZE
	unsigned char *img = (unsigned char *)(h->fdec->plane[0]);
	//printf("qp: %d f_value: %d qbits: %d\n",qp,f_value,qbits);
	printf("begin to write frame %d\n",frameIndex);
	char temp[10];
	char *testString = "255\n";
	char *testW = "P2\n";
	char testH[40];
	char fileName[50];
	sprintf(testH,"%d %d\n",h->fdec->i_stride[0],h->fdec->i_lines[0]);
	sprintf(fileName,"fdec_image%d.pgm",frameIndex);
	FILE * pFile;
	char hex[10];
	...
	fwrite(testString,1,strlen(testString),pFile);
  	for(i=0;i<(h->fdec->i_stride[0]*h->fdec->i_lines[0]);i++){
  	...
  	fclose (pFile);
	printf("finished writing to frame %d\n",frameIndex);
	if(frameIndex == 25) exit(0);
   #endif*/
...
//      fprintf(stderr, "current: %p  prev: %p  oldest: %p \n", thread_current, thread_prev, thread_oldest);
    ...
    *pp_nal = NULL;
    ...
    if( pic_in != NULL )
    {
       ...
       // printf("pic_in get fenc: %d\n",fenc);
        // start gpu compulation here (ERICH)

        if( x264_frame_copy_picture( h, fenc, pic_in ) < 0 )
            return -1;
            
           /* char *img = fenc->plane[0];
	char *testW = "P2\n";
	char testH[40];//= "1920 1080\n";
	sprintf(testH,"%d %d\n",fenc->i_stride[0],fenc->i_lines[0]);
	char *testString = "255\n";
	 FILE * pFile;
  	pFile = fopen ( "source2_image.pgm" , "wb" );
  	fwrite(testW,1,strlen(testW),pFile);
	fwrite(testH,1,strlen(testH),pFile);
	fwrite(testString,1,strlen(testString),pFile);
	char hex[10];
	printf("read height: %d width: %d\n",fenc->i_stride[0],fenc->i_lines[0]);
	int i;
  	for(i=0;i<(fenc->i_stride[0]*fenc->i_lines[0]);i++){
  		sprintf(hex,"%3d", (unsigned char)img[i]);
		fwrite(hex,1,strlen(hex),pFile);
  		//fputc((int)',',pFile);
  		//fputc((unsigned char)img[i],pFile);
  		if(i % fenc->i_width[0] == 0 && i > 0) fputc('\n',pFile);
  		else fputc(' ',pFile);
    ...
    if( h->frames.current[0] == NULL )
    {
		...
		// erich: decide the current slice type must be known
    ...
    //printf("before slice write fenc: %d\n",h->fenc);
    if( h->fenc == NULL )
    {
    ...
    x264_reference_build_list( h, h->fdec->i_poc );
     //printf("reframe before slice write fenc: %d\n",h->fref0[0]);
     

       #ifdef GPU_OPTIMIZE
    x264_gpu_t* gpu = (x264_gpu_t *)h->gpu;
    x264_gpu_thread_t* gpuThread = (x264_gpu_thread_t *)h->gpu_thread;
    //if(h->fenc != NULL && h->fref0[0] != NULL) {
	if(gpuThread->running == 0) {
		x264_gpu_thread_start(gpuThread);
		printf("thread started\n");
	}
	gpuThread->result = 0;
	//x264_timer_start();
        /*x264_gpu_processImage((x264_gpu_t *)h->gpu,h->fref0[0],h->fenc,0);
        x264_gpu_doSelection(gpu,0);*/
	/*x264_timer_stop();
	x264_print_time();*/
    //}
    #endif
