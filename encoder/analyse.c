...
static void x264_mb_analyse_load_costs( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    if( !p_cost_mv[a->i_lambda] )
    {
        ...
        // create the bit cots for the reference index
        for( i = 0; i < 3; i++ )
            for( j = 0; j < 33; j++ )
...
/*
 * 	Mode 0 (vert.) 	Extrapolation from upper samples (H)
	Mode 1 (hor.) 	Extrapolation from left samples (V)
	Mode 2 (DC)		Mean of upper and left-hand samples (H + V).
	Mode 4 (Plane) 	A linear �plane� function is �tted to the upper and left-hand samples H
					and V. This works well in areas of smoothly-varying luminance.

	see H.264 and MPEG-4 Visual Book p205, 6.4.6.2
 */
...
/*
 * intra ... macroblock will be predicted from prev. encoded and reconstructed macroblocks
 * 			  prior to filtering
 * 			 all maroblocks must be in the same frame ... no ref. to prev. encoded frames
 */
static void x264_mb_analyse_intra( x264_t *h, x264_mb_analysis_t *a, int i_satd_inter )
{
    const unsigned int flags = h->sh.i_type == SLICE_TYPE_I ? h->param.analyse.intra : h->param.analyse.inter;
    // pointer to the current encoded frame -> source pointer
    uint8_t  *p_src = h->mb.pic.p_fenc[0];
    // pointer to prev. encoded an reconstructed macroblocks
    ...
    // check which macroblock are available and therefore which intra modes can be used
    predict_16x16_mode_available( h->mb.i_neighbour, predict_mode, &i_max );

    if( b_merged_satd && i_max == 4 )
    {
    	// if intra_mbcmp_x3_16x16 method is available -> defined in pixel.c depending
		// on the current underlying architecture
        h->pixf.intra_mbcmp_x3_16x16( p_src, p_dst, a->i_satd_i16x16_dir );
        h->predict_16x16[I_PRED_16x16_P]( p_dst );
        a->i_satd_i16x16_dir[I_PRED_16x16_P] =
            h->pixf.mbcmp[PIXEL_16x16]( p_dst, FDEC_STRIDE, p_src, FENC_STRIDE );

        // calc costs for all four 16x16 intra modes
        // in the end choose the one which has a minimum cost value
        for( i=0; i<4; i++ )
        {
    ...
    {
    	// do the same calculation as above but use simple mbcmp instead
        for( i = 0; i < i_max; i++ )
        {
            int i_satd;
            int i_mode = predict_mode[i];
			...
			// call satd 16x16 function or normal sad function defined in pixel.c (PIXEL_SAD_C)
            // satd (PIXEL_SATD_C) defined in pixel.c too
            i_satd = h->pixf.mbcmp[PIXEL_16x16]( p_dst, FDEC_STRIDE, p_src, FENC_STRIDE ) +
                    a->i_lambda * bs_size_ue( x264_mb_pred_mode16x16_fix[i_mode] );
            // bs_size_ue returns the bits needed to transfer data
            // choose the minimum of all options COPY IF LESS THENjuuuuu
...
static void x264_mb_analyse_inter_p16x16( x264_t *h, x264_mb_analysis_t *a )
{
	...
	LOAD_HPELS( &m, h->mb.pic.p_fref[0][i_ref], 0, i_ref, 0, 0 );
        ...
        // calc predicted motion vector from prev. encoded motion vectors -> based on defined model (left(A), top(B) etc.)
        x264_mb_predict_mv_16x16( h, 0, i_ref, m.mvp );
        // predict references from prev. mb which one is the one that matches best
        x264_mb_predict_mv_ref16x16( h, 0, i_ref, mvc, &i_mvc );
        // search for mb in a given region
        // shall exchange the cpu code for the gpu code
        x264_me_search_ref( h, &m, mvc, i_mvc, p_halfpel_thresh );
	//printf("%3d %3d mvp: %5d %5d \n",h->mb.i_mb_x,h->mb.i_mb_y,m.mv[0],m.mv[1]);
        
        #else 
	/*
        x264_gpu_t *gpu = (x264_gpu_t *)h->gpu;
        x264_gpu_read_result(gpu);
        // calc predicted motion vector from prev. encoded motion vectors -> based on defined model (left(A), top(B) etc.)
        x264_gpu_mb_predict_mv_16x16( h, gpu, 0, i_ref, m.mvp );
        // predict references from prev. mb which one is the one that matches best
        //x264_gpu_mb_predict_mv_ref16x16( h, gpu, 0, i_ref, mvc, &i_mvc );
        // search for mb in a given region
        // shall exchange the cpu code for the gpu code
        x264_gpu_me_search_ref( h, gpu, &m, i_mvc, p_halfpel_thresh );*/
	x264_gpu_thread_t* gpuThread = (x264_gpu_thread_t *)h->gpu_thread;
	//if(gpuThread->result == 0) printf("UPDATE!!!!!!!!!!!\n");
	while(gpuThread->result == 0) {
		x264_gpu_thread_pop_result(h,gpuThread);
		pthread_yield();
	}

        x264_gpu_mb_predict_mv_16x162( h, gpuThread, 0, i_ref, m.mvp );
        x264_gpu_me_search_ref2( h, gpuThread, &m, i_mvc, p_halfpel_thresh );
	//printf("%3d %3d mvp: %5d %5d \n",h->mb.i_mb_x,h->mb.i_mb_y,m.mv[0],m.mv[1]);
...
void x264_macroblock_analyse( x264_t *h )
{
    ...
    else if( h->sh.i_type == SLICE_TYPE_P )
    {
        ...
        {
        	const unsigned int only_16x16 = 1;
            ...
            if( flags & X264_ANALYSE_PSUB16x16 && !only_16x16)
            {
            ...
            if( ( flags & X264_ANALYSE_PSUB16x16 ) &&
                analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost  && !only_16x16)
            {
                i_type = P_8x8;
                i_partition = D_8x8;
            ...
            if( ( flags & X264_ANALYSE_PSUB16x16 ) &&
                analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost + i_thresh16x8  && !only_16x16)
            {
