...
static ALWAYS_INLINE int x264_quant_4x4( x264_t *h, int16_t dct[4][4], int i_qp, int i_ctxBlockCat, int b_intra, int idx )
{
	...
	/*if(i_quant_cat == CQM_4IY) {
		int i;
		printf("qp: %d\n",i_qp);
		for(i=0;i<16;i++) {
			printf("mf(%d) %d | ",i,h->quant4_mf[i_quant_cat][i_qp][i]);
		}
		printf("\n");
		for(i=0;i<16;i++) {
			printf("f(%d) %d | ",i,h->quant4_bias[i_quant_cat][i_qp][i]);
		}
			exit(0);
	}*/
...
/*****************************************************************************
 * x264_macroblock_encode: ERICH ... must be modified ... to guarantee good behavior
 *****************************************************************************/
...
#else
	    DECLARE_ALIGNED_16( int nz_gpu[16]);
	    x264_gpu_thread_t* gpuThread = (x264_gpu_thread_t *)h->gpu_thread;
	    x264_gpu_inter_sub16x16_dct(dct4x4,h,gpuThread,nz_gpu);
#endif
