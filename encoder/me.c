...
#define CHECK_MVRANGE(mx,my) ( mx >= mv_x_min && mx <= mv_x_max && my >= mv_y_min && my <= mv_y_max )
    ...
    // here we do see that mvp stores qpel ... that's why the max./min. fullpel positions
    // do have to be multiplied with four
    bmx = x264_clip3( m->mvp[0], mv_x_min*4, mv_x_max*4 );
    bmy = x264_clip3( m->mvp[1], mv_y_min*4, mv_y_max*4 );
    // do round to full pel position
    pmx = ( bmx + 2 ) >> 2;
    pmy = ( bmy + 2 ) >> 2;
    bcost = COST_MAX;

    /* try extra predictors if provided */
    // i_subpel_refine is param --subme
    ...
    case X264_ME_DIA:
        ...
        do
        {
            ...
            COPY1_IF_LT( bcost, (costs[0]<<4)+1 );			//bmxy  1 (0,-1)	top			28(0)30(1)
            COPY1_IF_LT( bcost, (costs[1]<<4)+3 );			//bmxy 	3 (0,1)		bottom		28(0)30(3)
            COPY1_IF_LT( bcost, (costs[2]<<4)+4 );			//bmxy	1 (-1,0)	left		28(1)30(0)
            COPY1_IF_LT( bcost, (costs[3]<<4)+12 );			//bmxy	3 (1,0)		right		28(3)30(0)
            // if there was no match than the last 4 bits are zero and the loops ends
            if( !(bcost&15) )
                break;
            // what is the in-detail correlation between the calculated costs
			// and the bmx(y) values
            bmx -= (bcost<<28)>>30;
            bmy -= (bcost<<30)>>30;
			// remove last 4 bits
            bcost &= ~15;
