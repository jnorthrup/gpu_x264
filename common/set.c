...
int x264_cqm_init( x264_t *h )
{
    ...
    for( q = 0; q < 52; q++ )
    {
		...
		//printf("qp: %d bias(for): (%d) %d deadzone(%d)\n",q,i,h->quant4_bias[i_list][q][i],deadzone[i_list]<<10);
	...
	for(i=0;i<16;++i) printf("%d ",h->quant4_mf[1][q][i]);
	printf("\n");
	for(i=0;i<16;++i) printf("%d ",h->quant4_bias[1][q][i]);
	printf("\n");
