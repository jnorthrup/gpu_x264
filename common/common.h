...
static struct tms _timer_start, _timer_end;
clock_t c0, c1;

static void x264_timer_start() {
	c0 = clock();
	times(&_timer_start);
}
static void x264_timer_stop() {
	c1 = clock();
	times(&_timer_end);
}

static float x264_timer_elapsed_time() {
	return ((float)(_timer_end.tms_utime-_timer_start.tms_utime))/(1.0f*CLK_TCK);
}

static void x264_print_time() {
#undef printf
	long sum = ((long)c1-c0);
	if(sum == 0) return;
	printf("#####TIMER_OUTPUT ######\n");
	printf("clocks: %ld time: %f s\n",((long)c1-c0),(float)((float)c1-c0)/CLOCKS_PER_SEC);
	printf("user time=%f\n", ((float)(_timer_end.tms_utime-_timer_start.tms_utime))/(1.0f*CLK_TCK));
	printf("system time=%f\n",((float)(_timer_end.tms_stime-_timer_start.tms_stime))/(1.0f*CLK_TCK));
	printf("##################\n");
}
