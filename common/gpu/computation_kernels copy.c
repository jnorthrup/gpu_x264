...
((value + diff) > max_value) ? (max_value - value) : diff
	...
	int startRegion = offset-(min(mb_x*mb_width,search_region));
	startRegion -= (min(mb_y*mb_height*image_width,search_region*image_width));
	...
	int ax	= delta_region_width*(ass_number%GROUP_WRAP);// + delta_region_width>>1; 
	int ay	= delta_region_height*(ass_number/GROUP_WRAP);// + delta_region_height>>1; 
	
	int mvp_x = ax;
	int mvp_y = ay;
	
	local_result[ass_number].mv_predicted.x = mvp_x;
	local_result[ass_number].mv_predicted.y = mvp_y;
	local_result[ass_number].refIdx = refIdx;
	local_result[ass_number].mv_diff.x = 0;
	local_result[ass_number].mv_diff.y = 0;
	local_result[ass_number].costs = COST_MAX;
	
	
	int cost = 0;
	// calc SAD
	int start_region = mad24(ay,temp_width,ax);
	local_result[ass_number].mv_diff.x = start_region;
	int mvd_y_n, mvd_x_n;
	...
	if(global_id == 0) {
		...
		for(y=0;y<mb_height;y++) {
			for(x=0;x<mb_width;x+=4) {
				...
				cost += abs_diff(current_mb[mvd_x_n+1],region_buffer[start_region+x+1]);
				cost += abs_diff(current_mb[mvd_x_n+2],region_buffer[start_region+x+2]);
				cost += abs_diff(current_mb[mvd_x_n+3],region_buffer[start_region+x+4]);
				mvd_x_n+=4;
			}
		...
		local_result[ass_number].costs = cost;
		curCost[ass_number] = cost;
	}
...
#define COST_INDEX 2
		...
		if(neighbour[i+Y_INDEX] > -1 && neighbour[i+X_INDEX] > -1) {
			...
			// calc SAD
			start_region = mad24(neighbour[i+Y_INDEX],temp_width,neighbour[i+X_INDEX]);
			neighbour[i+COST_INDEX] = 0;
			mvd_x_n = 0;
			for(y=0;y<mb_height;++y) {
				for(x=0;x<mb_width;x+=4) {
					neighbour[i+COST_INDEX] += abs_diff(current_mb[mvd_x_n+0],region_buffer[start_region+x]);
					neighbour[i+COST_INDEX] += abs_diff(current_mb[mvd_x_n+1],region_buffer[start_region+x+1]);
					neighbour[i+COST_INDEX] += abs_diff(current_mb[mvd_x_n+2],region_buffer[start_region+x+2]);
					neighbour[i+COST_INDEX] += abs_diff(current_mb[mvd_x_n+3],region_buffer[start_region+x+3]);
					mvd_x_n+=4;
				}
				start_region += temp_width-mb_width;
			}
