...
{
    int i_mb_xy = i_mb_y * h->mb.i_mb_stride + i_mb_x;				// 16 x 16 blocks
    int i_mb_4x4 = 4*(i_mb_y * h->mb.i_b4_stride + i_mb_x);			// 4 x 4 blocks
    int i_mb_8x8 = 2*(i_mb_y * h->mb.i_b8_stride + i_mb_x);			// 8 x 8 blocks
    int i_top_y = i_mb_y - (1 << h->mb.b_interlaced);				// load top y location macroblock
    int i_top_xy = i_top_y * h->mb.i_mb_stride + i_mb_x;			// load top macroblock position
    ...
    /* load cache */
    if( i_top_xy >= h->sh.i_first_mb )			/* if top macroblock is available */
    {
        h->mb.i_mb_type_top =
        i_top_type= h->mb.type[i_top_xy];		/* load top macroblock type */
        h->mb.cache.i_cbp_top = h->mb.cbp[i_top_xy];		/* cache cbp ... coded block pattern */

        h->mb.i_neighbour |= MB_TOP;			/* update available neighboirs*/
    ...
    else			/* top mb is not available ... set to default values .... see h.264 standard */
    {
    ...
    if( i_mb_x > 0 && i_mb_xy > h->sh.i_first_mb )			/* left mb is available */
    {
        i_left_xy = i_mb_xy - 1;
        h->mb.i_mb_type_left =
