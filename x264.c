...
int main( int argc, char **argv )
{
	...
	//test_x264_gpu_calc_cost_p16x16();
...
#define OPT_LONGHELP 263
	...
	fileName = (char *)malloc(strlen(psz_filename));
	strcpy(fileName,psz_filename);
    /* check demuxer type */
...
#ifdef GPU_OPTIMIZE  
    // init gpu context
    h->gpu = malloc(sizeof(x264_gpu_t));
    h->gpu_thread = malloc(sizeof(x264_gpu_thread_t));
    x264_gpu_thread_t* gpuThread = (x264_gpu_thread_t *)h->gpu_thread;
    x264_gpu_t *gpu = (x264_gpu_t *)h->gpu;
	/* see frame.c x264_frame_new PADH for more information */
    gpu->image_info.image_width = param->i_width;
	gpu->image_info.image_height = param->i_height;
	gpu->image_info.search_region = 16;
	gpu->image_info.mb_width = 16;
	gpu->image_info.mb_height = 16;
	gpu->image_info.refIdx = 0;
    x264_gpu_init(gpu);
    x264_init_me_gather_kernel(gpu);
    x264_init_me_selection_kernel(gpu);
    x264_init_me_reset_grid(gpu);
    x264_init_gpu_thread(h,gpu,gpuThread,fileName);
    gpuThread->qp = param->rc.i_qp_constant;
#endif

    /* Encode frames */
    for( i_frame = 0, i_file = 0; b_ctrl_c == 0 && (i_frame < i_frame_total || i_frame_total == 0); )
    {
	//printf("START COMPUTATION FOR FRAME %d\n",i_frame);
...
#ifdef GPU_OPTIMIZE
	printf("PREPROCESSING OUTPUT: frame misses: %d frame hits: %d\n",gpuThread->miss_count,gpuThread->hit_count);
	x264_gpu_thread_stop(gpuThread);
#endif
